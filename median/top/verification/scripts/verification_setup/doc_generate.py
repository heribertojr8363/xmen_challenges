...
# 
#  File Name         : doc_generate.py
# 
#  Author            : Jose Iuri B. de Brito (XMEN LAB), Matheus Maciel de Sousa (XMEN LAB)
# 
#  Purpose           : This File is used to generate latex files to
#                      documentation.   
######################################################################
...
def doc_generate(author, module, interface, interface_path, test, tb_path, passive, rtl_path):
...
def doc_generatea(author, module, interface, interface_path, test, doc_path, passive, rtl_path):
...
\\usepackage{{graphicx}}
\\graphicspath{{{{{PATH}}}}} % Setting the graphicspath
...
\\pagestyle{{fancy}}
\\rhead{{\\leftmark}}
\\lhead{{Guia de Verificação - Bloco {MODULE}}}
...
\\title{{Guia de Verificação - Bloco {MODULE}}}
%---------------------------------------------------------------------------------------
...
\\begin{{versionhistory}}
  \\vhEntry{{1.0}}{{{DATE}}}{{{AUTOR}}}{{created}}
\\end{{versionhistory}}
...
\\par
Esse relatório tem o objetivo de apresentar o desenvolvimento de uma arquitetura de \\textit{{testbench}} para a verificação funcional baseada na metodologia UVM do bloco {MODULE}.
...
\\begin{{table}}[h]
\\caption{{Nomenclaturas e Representação de Bits e Amostras}}
\\centering
\\resizebox{{\\textwidth}}{{!}}{{%
\\begin{{tabular}}{{|
...
\\begin{{figure}}[h]
    \\centering
        ...
        \\caption{{Arquitetura do testbench do {MODULE}}}
        \\label{{diagrama}}
\\end{{figure}}
...
commit 3213213213546510321358465 (origin/{MODULE}/Verification/tb)
...
\\end{{document}}""".format(MODULE = module, AUTOR = author, DATE=arrow.now().format('YYYY-MM-DD'), PATH=os.path.dirname(os.path.realpath(__file__))+'/'+'resources/')

    file = doc_path / """{MODULE}.tex""".format(MODULE=module)
    file1 = open(file,"a+")
    file1.write(doc)
    file1.close()